    ...
    internal class Student
    {
        public static void Main()
        {
            IList<StudentDemo> students = new List<StudentDemo>()
           {
            ...
            var groupJoin = courseList.GroupJoin(students, st => st.ID, s => s.StudentCourseId,
                (st, studGrp) => new { Stu = studGrp, CfN = st.CourseName });

            foreach (var item in groupJoin)
            {
                Console.WriteLine(item.CfN);
                foreach (var stud in item.Stu)
                {
